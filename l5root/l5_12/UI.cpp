	...
	while (true)
	{
		...
		switch (option)
		{
			...
			case 2:
			{
				...
				contr.getRepo().del(old_id, 1); // sterg din baza de date filmul precedent

				ask_get_ma(id, title, genre, year, likes, trailer);
				//id_validation_screen(id, 2, old_id);
				
				year_validation_screen(year);
				likes_validation_screen(likes);
				
				Movie new_movie(id, title, genre, year, likes, trailer);
				//contr.getRepo().add(new_movie, 1);
	...
	while (true)
	{
		...
		switch (option)
		{
			...
			case 2:
			{
				...
				if (!watchlist.size()) { afis_lista(2); break; }
				afis_lista(2);
				string id;
