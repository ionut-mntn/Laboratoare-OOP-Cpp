...
bool Validator::id_exists(const string &id, vector<string> &ids_list)
{
	int l = ids_list.size();
	for (int i = 0; i < l; i++)
		if (id == ids_list[i]) return true;
	return false;
}

bool Validator::id_does_not_exist(const string& id, vector<string> &ids_list)
{
