...
//	 string user_watchlist_filename;
	 ...
	 /* Method that reads */
	 vector<Movie> read_database(string file = "");

	 /* Returns name of: database file or user watchlist file, based on the reference passed to the function! */
	 string get_filename(const vector<Movie>&) const;
	 
	 /* scrie modificarile si in fisier */
	 void write_to_file(const string & file, const vector<Movie> &) const; // operatorii `<<` si `>>` nu schimba obiectul?

public:
	
	/* Constructor */
	Repository(string database_file = "Movies Database.txt", string watchlist_file = "User Watchlist Database.txt");
	...
	*/
	...
	string toString(const vector<Movie> &) const;
