...
void UI::run()
{
	int mode, user_watchlist_mode;
	vector<Movie> user_watchlist;
	
	while (true)
	{
		select_user_watchlist_file_type_screen();
		ask_get("user's watchlist file type", user_watchlist_mode);
...
void UI::afis_lista(const vector<Movie> & list)
{
	...
	//cout << list << '\n';
	cout << contr.getRepo().toString(list);
}
...
void UI::select_user_watchlist_file_type_screen()
{
	cout << "Select user's watchlist type of file:\n"
		<< "1.CSV (Comma Sparated Values\n"
